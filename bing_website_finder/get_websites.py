...
from bing_website_finder.new_obj import WebsiteWorker, EmailWorker
...
def init(infilepth, outfilepth, verbose=False, api_key=None, num_workers=5):
    ...
    loop = asyncio.get_event_loop()
    df_to_company_db(cache)
    try:
        loop.run_until_complete(_execute_website_workers(workers, verbose, loop=loop))
    finally:
